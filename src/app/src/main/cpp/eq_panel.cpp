...
void buildEqVerts(std::vector<float> &v, int hoverBand, bool resetHover,
    ...
    // RESET button (centred, between the faders/readout and the preset dropdown).
    {
        ...
        appendTextLine(tmp, "Reset", (kEqResetYTop+kEqResetYBot)*0.5f + gFont.lineHeight()*rpx*FontAtlas::pxScale()*0.5f, rpx, 1,1,1);
    ...
    // Preset dropdown header.
    {
        ...
        appendTextLine(v, hbuf, (kEqPresetYTop+kEqPresetYBot)*0.5f + gFont.lineHeight()*px*FontAtlas::pxScale()*0.5f, px, 1, 1, 1);
    }
    ...
    if (gEqPresetOpen) {
        for (int i = 0; i < kEqNumPresets; i++) {
            ...
            appendTextLine(v, kEqPresetNames[i], (yT+yB)*0.5f + gFont.lineHeight()*px*FontAtlas::pxScale()*0.5f, px, 1, 1, 1);
        }
    }
}
