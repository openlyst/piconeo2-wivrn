...
void queueHaptic(int hand, float amplitude, float frequency, float durationS) {
    ...
    if (amplitude <= 0.0f || durationS <= 0.0f) return;
    if (amplitude > 1.0f) amplitude = 1.0f;
    // Clamp to a sane on-device window: floor very short pulses so the motor fires,
    // cap long ones so a stuck event can't buzz forever.
