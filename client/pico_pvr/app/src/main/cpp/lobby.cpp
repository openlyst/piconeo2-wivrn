...
{
    gl_Position = u_mvp * vec4(a_pos, 1.0);
    v_uv = (u_tex_matrix * vec4(a_uv, 0.0, 1.0)).xy;
}
)";
...
void pico_lobby::init(int w, int h)
{
	...
	tex_matrix_uniform = glGetUniformLocation(tex_program, "u_tex_matrix");
...
void pico_lobby::set_surface_texture(JNIEnv *env, jobject st, jmethodID update_method)
{
	surface_texture = st;
	...
	if (env && st)
	{
		env->GetJavaVM(&tex_vm);
		jclass stClass = env->GetObjectClass(st);
		if (stClass)
		{
			get_transform_matrix_method = env->GetMethodID(stClass, "getTransformMatrix", "([F)V");
                           ...
                           const XrFovf & fov, float ipd, int eye)
{
	...
	glUniformMatrix4fv(tex_matrix_uniform, 1, GL_FALSE, tex_matrix);
...
{
	if (!frame_available.load(std::memory_order_relaxed) || !surface_texture || !update_tex_image_method || !tex_vm)
		return;

	JNIEnv *env = nullptr;
	bool attached = false;
	if (tex_vm->GetEnv((void **)&env, JNI_VERSION_1_6) != JNI_OK)
	{
		if (tex_vm->AttachCurrentThread(&env, nullptr) == JNI_OK)
			attached = true;
		else
			return;
	}

	update_tex_image(env);

	if (get_transform_matrix_method)
	{
		jfloatArray matArr = env->NewFloatArray(16);
		if (matArr)
		{
			env->CallVoidMethod(surface_texture, get_transform_matrix_method, matArr);
			env->GetFloatArrayRegion(matArr, 0, 16, tex_matrix);
			env->DeleteLocalRef(matArr);
		}
	}

	if (attached)
		tex_vm->DetachCurrentThread();
}

void pico_lobby::update_texture(int, int, const void *) {}
