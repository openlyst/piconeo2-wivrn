...
extern jclass g_vr_class;

static void call_vr_bootstrap(JavaVM * vm)
{
	if (!g_vr_class || !vm)
		return;
	...
	jmethodID m = env->GetStaticMethodID(g_vr_class, "removePlatformLogo", "()V");
	if (m)
	{
		env->CallStaticVoidMethod(g_vr_class, m);
		spdlog::info("called VrActivity.removePlatformLogo");
	}
	else
	{
		env->ExceptionClear();
		spdlog::warn("VrActivity.removePlatformLogo not found");
	}
...
void pico_render_thread::init_sdk()
{
	...
	jclass vrClass = g_vr_class;
	if (!vrClass && env && client->activity)
	{
...
void pico_render_thread::run()
{
	...
	while (running.load())
	{
		...
		static int bootstrap_frames = 0;
		if (rt_inited && ++bootstrap_frames == 3 && client && client->vm)
			call_vr_bootstrap(client->vm);
